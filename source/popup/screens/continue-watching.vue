<template>
  <div class="h-full flex flex-col">
    <div class="flex-grow overflow-hidden">
      <div class="h-full flex" v-if="videos.length === 0">
        <div class="m-auto px-8 text-center font-italic text-gray-300">
          ...
          <template v-else-if="failed">
            ...
            <div class="text-base">Loading videos failed!</div>
            <div class="mt-1">
      ...
      <div v-else class="h-full overflow-y-auto overflow-x-hidden">
        ...
        >
          <video-item v-for="video in videos" :key="video.id" :video="video" />
        </transition-group>
      </div>
    </div>
    <div
...
import fetchMyVideos, {
  getCachedVideos,
  Video,
} from '../../libs/fetch-my-videos'
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    if (props.settings.region) {
      getCachedVideos().then(newVideos => (videos.value = newVideos))
